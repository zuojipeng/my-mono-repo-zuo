...
export default function CourseContentViewer({
  ...
  isOpen,
  onClose
}: CourseContentViewerProps): React.ReactElement | null {
  const { address } = useAccount();

  // 检查是否是创建者
  const isCreator: boolean = !!address && !!courseAuthor &&
  ...
  // 检查权限
  const hasPermission: boolean = isCreator || hasPurchased === true;
  ...
  const handleDownload = async (): Promise<void> => {
    ...
    try {
      ...
      // 从 IPFS 下载文件
      const response: AxiosResponse<Blob> = await axios.get(ipfsUrl, {
        responseType: 'blob',
