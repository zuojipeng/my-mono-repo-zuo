...
// 类型定义
type StatusType = 'idle' | 'signing' | 'waiting' | 'success' | 'error';
...
export default function CreateCourseModal({ isOpen, onClose, onSuccess }: CreateCourseModalProps): React.ReactElement | null {
  ...
  const handleContentUploadSuccess = (result: UploadResult | string): void => {
    // console.log('收到上传成功回调，result:', result);
    const hash = typeof result === 'string' ? result : (result?.ipfsHash || result?.hash);
    if (hash) {
      setContentHash(hash);
  ...
  const handleThumbnailUploadSuccess = (result: UploadResult): void => {
    if (result.ipfsHash) {
