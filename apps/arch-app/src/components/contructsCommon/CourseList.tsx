...
import { COURSE_PURCHASE_ADDRESS, COURSE_PURCHASE_ABI } from '../../../costruct.config';
import type { Course } from '../../types/course';

interface CourseListProps {
  onSelectCourse?: (course: Course) => void;
  onPurchaseClick?: (course: Course) => void;
  onViewContent?: (course: Course) => void;
}
 ...
 * - 支持查看课程内容（仅限已购买或创建者）
 */
export default function CourseList({ onSelectCourse, onPurchaseClick, onViewContent }: CourseListProps): React.ReactElement {
  const { courses, isLoading, isError } = useCourses();
  const { address } = useAccount();

  const isOwnCourse = (instructor: string): boolean => {
  ...
  const purchaseCheckContracts = courses?.map((course) => ({
    address: COURSE_PURCHASE_ADDRESS as `0x${string}`,
  ...
  // 创建课程ID到购买状态的映射
  const purchaseStatusMap: Record<string, boolean> = {};
  if (purchaseStatuses && courses) {
    courses.forEach((course, index) => {
      if (purchaseStatuses[index]?.status === 'success') {
        purchaseStatusMap[course.id] = purchaseStatuses[index].result as boolean;
      }
    });
  }

  // 检查用户是否有权限查看课程内容
  const canViewContent = (course: Course): boolean => {
    if (!address) return false;
    // 如果是创建者，可以查看
