...
// 类型定义
type StepType = 'idle' | 'checking' | 'needApprove' | 'approving' | 'purchasing' | 'success' | 'error';
...
const STEP: Record<string, StepType> = {
  IDLE: 'idle',
  CHECKING: 'checking',
...
export default function PurchaseCourseModal({
  ...
  course,
  onSuccess,
}: PurchaseCourseModalProps): React.ReactElement | null {
  const { address } = useAccount();
  const publicClient = usePublicClient();

  const [step, setStep] = useState<StepType>(STEP.IDLE);
  const [errorMessage, setErrorMessage] = useState<string>('');
  const handledTxRef = useRef<Hash | null>(null);
  const manualCheckRef = useRef<boolean | null>(null); // 用于手动检查交易
  ...
  const handleClose = useCallback((force?: boolean): void => {
    const busy =
      step === STEP.APPROVING ||
  ...
  const manuallyCheckTransaction = useCallback(async (txHash: Hash): Promise<void> => {
    if (!txHash) {
    ...
    try {
      ...
      const errorMessage = err && typeof err === 'object' && 'message' in err
        ...
        : err?.toString() || '未知错误';
      setErrorMessage(`交易确认失败: ${errorMessage}`);
    }
  }, [publicClient, onSuccess, handleClose]);

  // 使用本地状态存储交易哈希（作为备用）
  const [localPurchaseTxHash, setLocalPurchaseTxHash] = useState<Hash | null>(null);
