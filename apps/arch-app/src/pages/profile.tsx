...
function ProfilePage() {
  ...
  const [formError, setFormError] = useState<string>(''); // 表单验证错误

  useEffect(() => {
    document.title = '个人中心 - Web3大学';
    setMounted(true);
  }, []);
  ...
  const { data: profileData, refetch: refetchProfile } = useReadContract({
    address: USER_PROFILE_ADDRESS as Address,
  ...
  useEffect(() => {
    ...
    const fetchedUsername = (profileData as any).name || (profileData as any)[0] || '';
  ...
  // 批量检查购买状态
  const purchaseCheckContracts = ((allCourseIds as bigint[]) || []).map((courseId) => ({
    address: COURSE_PURCHASE_ADDRESS as Address,
  ...
  const { data: purchaseStatuses } = useReadContracts({
    ...
    query: { enabled: mounted && !!address && allCourseIds && (allCourseIds as bigint[]).length > 0 },
  });

  // 获取已购买课程的详情
  const purchasedCourseIds = ((allCourseIds as bigint[]) || []).filter((_, index) => {
  ...
  useEffect(() => {
    ...
    const courses = courseDetails.map((result, index) => {
      ...
      const course = result.result as any;
      ...
      return {
        ...
        priceDisplay: course.price ? formatUnits(course.price as bigint, 18) : '0',
            ...
            {writeError && (
              <div className="bg-red-900/20 border border-red-500 rounded-lg p-3 mt-3">
                <p className="text-red-400 text-sm">
                  ❌ {(writeError as any)?.shortMessage || (writeError as any)?.message || '交易失败'}
                </p>
              </div>
            )}
