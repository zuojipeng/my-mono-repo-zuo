...
import {
  YD_TOKEN_ADDRESS,
  YD_TOKEN_ABI,
} from '../../costruct.config.js';
import { CURRENT_CHAIN_ID, IS_LOCAL_CHAIN } from '@zuojipeng/my-libs/wagmi';
...
import CourseList from '@/components/contructsCommon/CourseList';
...
export default function Home(): JSX.Element {
  ...
  const [isContentViewerOpen, setIsContentViewerOpen] = useState<boolean>(false);
  const [selectedCourse, setSelectedCourse] = useState<Course | null>(null);
  const [viewingCourse, setViewingCourse] = useState<Course | null>(null);
  const [refreshKey, setRefreshKey] = useState<number>(0);
  const [isSwitching, setIsSwitching] = useState<boolean>(false); // 切换网络中的简单状态
  ...
  // 添加目标网络到 MetaMask
  const addTargetNetwork = async (): Promise<boolean> => {
    if (typeof window === 'undefined' || !window.ethereum) {
      alert('请安装 MetaMask 钱包');
      return false;
    }

    const networkConfig: NetworkConfig = IS_LOCAL_CHAIN
      ? {
  ...
  // 手动切换到目标网络
  const switchToTargetNetwork = useCallback(async (): Promise<void> => {
    if (typeof window === 'undefined' || !window.ethereum) {
      alert('请安装 MetaMask 钱包');
      return;
    }
  ...
  const handleConnect = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        const chainNum = normalizeChainId(chainIdHex as string | number | bigint | undefined);
  ...
  const handlePurchaseClick = (course: Course): void => {
  ...
  const handleViewContent = (course: Course): void => {
    setViewingCourse(course);
    setIsContentViewerOpen(true);
  };

  const displayBalance: string = ydBalance ? formatUnits(ydBalance as bigint, 18) : '0';
