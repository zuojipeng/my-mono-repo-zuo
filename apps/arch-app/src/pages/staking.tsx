...
import { formatUnits, parseEther, Address, Hash } from 'viem';
import {
  useAccount,
  useConnect,
  useDisconnect,
...
import { YD_STAKING_ADDRESS, YD_STAKING_ABI } from '../../costruct.config';
...
function StakingPage() {
  ...
  const { data: allTiers, refetch: refetchTiers } = useReadContract({
    address: YD_STAKING_ADDRESS as Address,
    abi: YD_STAKING_ABI,
  ...
  const handleClaimReward = async (stakeId: bigint): Promise<void> => {
    try {
  ...
  const handleUnstake = async (stakeId: bigint, isEarly: boolean = false): Promise<void> => {
    try {
      if (isEarly) {
  ...
  const tiers = Array.isArray(allTiers) ? (allTiers as any[]) : [];
  const stakesRaw = activeTab === 'stake' ? activeStakes : userStakes;
  const stakes = Array.isArray(stakesRaw) ? (stakesRaw as any[]) : [];
  ...
  if (rewardsData && stakes.length > 0) {
    ...
    stakes.forEach((stake) => {
      ...
      if (stakeData.isActive && stakeData.stakeId && rewardsData[rewardIndex]) {
        const reward = rewardsData[rewardIndex].status === 'success' ? (rewardsData[rewardIndex].result as bigint) : 0n;
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {tiers.map((tier, index) => {
                      const tierData: TierData = Array.isArray(tier) ? {
                        minAmount: tier[0],
                        ydReward: tier[1],
                        lockPeriod: tier[2],
                        apy: tier[3],
            ...
            {!isConnected ? (
              ...
              <div className="space-y-4">
                ...
                {stakes.map((stake, index) => {
                  const stakeData: StakeData = Array.isArray(stake) ? {
                    stakeId: stake[0],
                    user: stake[1],
                    ethAmount: stake[2],
                    ydAmount: stake[3],
